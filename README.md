...
- 汇总：期初、期末、净变化、获取 / 消耗
- 按原因分类统计（映射见同目录 `change_reasons.json`；可手改，也可点「更新码表」从仓库热更）
- 明细默认折叠；可导出 CSV
- 汇总报告：预览后确认保存为 PNG
...
三币种共用同一套 `changeReason` 码，有两种更新方式：

1. **热更（推荐）**：点「自动更新变动原因码表」，在弹窗中选择 **从官网更新**（解析客服中心前端 JS）或 **从 GitHub 更新**；也可「手动更新变动原因码表」粘贴完整 JSON。
2. **手改**：直接编辑程序目录下的 `change_reasons.json`（`reasons` 对象）后保存，再查一次即可，**不必重新打包**。
...
`version` 建议用日期（`YYYY-MM-DD`），便于对照热更前后的码表。未收录的码会显示为「未知原因(码)」。
